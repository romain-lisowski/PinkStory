...
final class UserRepository extends AbstractRepository implements UserRepositoryInterface
{
    ...
    public function findOne(string $id): UserFull
    {
        ...
        $qb->addSelect('u.name as user_name', 'u.name_slug as user_name_slug', 'u.email as user_email')
            ->andWhere($qb->expr()->eq('u.id', ':user_id'))
            ->setParameter('user_id', $id)
        ;
    ...
    {
        $qb->select('u.id as user_id', 'u.image_defined as user_image_defined', 'u.created_at as user_created_at')
            ->from('usr_user', 'u')
