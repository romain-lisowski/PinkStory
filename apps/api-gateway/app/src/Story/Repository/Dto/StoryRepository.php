...
final class StoryRepository extends AbstractRepository implements StoryRepositoryInterface
{
    public function findOne(string $id): StoryFull
    {
        ...
        $qb->select('story.id as story_id', 'story.title as story_title', 'story.title_slug as story_title_slug', 'story.content as story_content', 'story.created_at as story_created_at', 'story.parent_id as story_parent_id')
            ...
            ->addSelect('u.id as user_id', 'u.image_defined as user_image_defined', 'u.name as user_name', 'u.name_slug as user_name_slug')
        ...
        $user = new UserMedium(strval($data['user_id']), boolval($data['user_image_defined']), strval($data['user_name']), strval($data['user_name_slug']), $userLanguage);

        $language = new LanguageMedium(strval($data['story_language_id']));

        $story = new StoryFull(strval($data['story_id']), strval($data['story_title']), strval($data['story_title_slug']), strval($data['story_content']), new DateTime($data['story_created_at']), $language, $user);
        $stories->add($story);
